...
tic;  
%Read images
[imd_f,ImagsInitial]=ReadImag(fPath);
...
[Rows, Columns] =Rows_Columns(imd_f);
;Time.TimeRows_Columns=toc;
%Create look up table
%Rows=15 y Columns=15
tic;
[lut,Rows,Columns]=lut(imd_f.Files,'left',Rows,Columns);
ImagMatrix = LUT_Stitch('left',Rows,Columns);
;Time.TimeLut=toc;
%Resize Imags
...
if exist(strcat(fPath,'\ImagResize\'),'dir')==0 && FlagScale~=false
    tic;
    [imd_Resize,ImagsResize]= Resize(imd_f,fPathResize,ScaleFactor);
    ;Time.TimeResize=toc;
elseif FlagScale~=false
    tic;
    [imd_Resize,ImagsResize]=ReadImag(fPathResize);
    ;Time.TimeResize=toc;
end
%Correct Vignneting
...
%     IND=[5,6,26,36];
    %Correct Image
    tic;
    [imds_Corrected,ImagsCorrected,im_correction_col, cropArea]=CorrectVignnetingColor(imd_Resize, fPathCorrect,IND,'VignetingCorrection_rgb.mat',ImagsResize);
    ;Time.TimeCorrectionVignetting=toc;
else
    imds_Corrected=ReadImag(fPathCorrect);
end
%Correct Exposition
if (exist(strcat(fPath,'\ImagCorrectExpos\'),'dir')==0) && (isequal(FlagExposition,'yes'))
    ...
    [ImagsExposCorrect, imd_Expos] = ExposCorrection(imds_Corrected,Params,Measure,fPathExpos,0.5,ImagsCorrected);
    ;Time.TimeCorrectionExposition=toc;
elseif (isequal(FlagExposition,'yes'))
     tic;
    [imd_Expos,ImagsExposCorrect]=ReadImag(fPathExpos);
    ;Time.TimeCorrectionExposition=toc;
end
%Correct Distorsion
...
elseif (isequal(FlagDistorsion,'yes'))
     tic;
    [imd_Distorted,ImagsDistorted]=ReadImag(fPathDistors);
    ;Time.TimeCorrectionExposition=toc;
end   
%Crop Image
if exist(strcat(fPath,'\ImagCrop1\'),'dir')==0
    if ~isequal(FlagExposition,'yes') && ~isequal(FlagDistorsion,'yes') 
        ...
        ImagsDistorted=ImagsCorrected;
    elseif ~isequal(FlagDistorsion,'yes') 
        imd_Distorted=imd_Expos;
        ImagsDistorted=ImagsExposCorrect;
    end
    %Type 1: Crop1
     tic;
    ...
    [imd_Cropped1,ImagsCropped1]=ReadImag(fPathCrop1);
end
    %Overlaps
...
if exist(strcat(fPath,'\ImageRename\'),'dir')==0
    %Rename Images
    [RenameImd,fPathRename]=Rename(ImagsCropped1,fPathRename);
    tic;
    %REGISTER
    %Type 1

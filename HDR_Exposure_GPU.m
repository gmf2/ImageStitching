function [Params,Measure]=HDR_Exposure_GPU(imd_f,fPath,OPEN_MEASURES)
...
for i=1:numel(imd_f.Files)
    ...
    %Params
    Params.ExposureTime{i}=gpuArray(info.DigitalCamera.ExposureTime);
    Params.ShutterSpeedValue{i}=gpuArray(info.DigitalCamera.ShutterSpeedValue);
    Params.ExposureBiasValue{i}=gpuArray(info.DigitalCamera.ExposureBiasValue);
    Params.FNumber{i}=gpuArray(info.DigitalCamera.FNumber);
    Params.ISOSpeedRatings{i}=gpuArray(info.DigitalCamera.ISOSpeedRatings);
    Params.FocalLength{i}=gpuArray(info.DigitalCamera.FocalLength);
end
    %Arrays

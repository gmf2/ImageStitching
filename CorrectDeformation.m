function [UndistorsionImage,I2] = CorrectDeformation(ImagsExposCorrect, k,fPathCorrectDist, varargin)
...
for j=1:numel(ImagsExposCorrect)
    I{j}=gather(ImagsExposCorrect{j});
%     I{j}=ImagsExpo{j};
    if ndims(I{j}) == 3
         for i=1:3
            I2{j}(:,:,i) = imdistcorrect(I{j}(:,:,i),k);
         end   
    elseif ismatrix(I)
        I2{j} = imdistcorrect(I{j},k);
    else
...
%Mkdir subfolder
%fPathDist=[fPathCorrectDist,'\ImagCorrectDistorsion\'];
mkdir(fPathCorrectDist)
 
for z=1:numel(I2)
    fNameImCorrected= [fPathCorrectDist ,'Imcorr_', num2str(z), '.jpg'];
    imwrite(I2{z}, fNameImCorrected);
end

UndistorsionImage=imageDatastore(fPathCorrectDist);
UndistorsionImage.Files = sort_nat(UndistorsionImage.Files,'ascend');
end
